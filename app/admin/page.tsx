...
import CajaPro from "@/components/CajaPro";
...
import {
  ...
  getCashboxTransactions, getCashboxSummary, addCashboxTransaction, deleteCashboxTransaction
} from "./actions";

export default function AdminPage() {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [activeTab, setActiveTab] = useState<"productos" | "categorias" | "reparaciones" | "usuarios" | "ventas" | "inventario" | "caja">("productos");
  ...
  const [newCashboxTx, setNewCashboxTx] = useState({ description: "", type: "INCOME", amount: "", paymentMethod: "EFECTIVO", notes: "" });


  // SECURITY FIX (VULN-08): Authentication guard state.
  // null = checking, true = authenticated, false = not authenticated (redirecting)
  const [isAuthenticated, setIsAuthenticated] = useState<boolean | null>(null);

  useEffect(() => {
    // Verify session on mount before rendering any admin content
    checkSessionAndLoad();
  }, []);
  ...
  const checkSessionAndLoad = async () => {
    try {
      const [p, c, r, u, o, s, st, cbTx, cbSum] = await Promise.all([
        getProducts(), getCategories(), getRepairs(), getUsers(), getOrders(), getSales(), getSaleStats(), getCashboxTransactions(), getCashboxSummary()
      ]);
      setProducts(p); setCategories(c); setRepairs(r); setAdminUsers(u); setOrders(o); setSales(s); setStats(st); setCashboxTransactions(cbTx); setCashboxSummary(cbSum);
      // SECURITY FIX (VULN-08): Only show admin UI after data loads successfully (means session is valid)
      setIsAuthenticated(true);
    } catch (err: any) {
      console.error("Error cargando datos:", err);
      // If auth error (session expired or invalid), redirect to login
      const message = err?.message || String(err);
      if (message.includes("autenticado") || message.includes("Sesión") || message.includes("No autenticado")) {
        setIsAuthenticated(false);
        router.push("/admin/login");
      } else {
        // Non-auth error (e.g. network) — still show UI but as authenticated (session might be valid)
        setIsAuthenticated(true);
      }
    }
  };
  ...
  // --- Handlers ---
  const handleAddCashboxTransaction = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCashboxTx.description || !newCashboxTx.amount) return;
    try {
      await addCashboxTransaction({
        ...newCashboxTx,
        amount: parseFloat(newCashboxTx.amount)
      });
      setNewCashboxTx({ description: "", type: "INCOME", amount: "", paymentMethod: "EFECTIVO", notes: "" });
      loadData();
    } catch {
  ...
  const handleDeleteCashboxTx = async (id: string) => {
  ...
  // SECURITY FIX (VULN-08): Authentication rendering guard.
  // Do not render ANY admin content until session is verified.
  if (isAuthenticated === null) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center space-y-4">
          <div className="w-12 h-12 border-4 border-blue-infositel border-t-transparent rounded-full animate-spin mx-auto" />
          <p className="text-gray-400 font-semibold text-sm uppercase tracking-widest">Verificando sesión...</p>
        </div>
      </div>
    );
  }
  ...
  if (isAuthenticated === false) {
    // Redirecting via router.push("/admin/login") — render nothing
    return null;
  }

  return (
    <div className="min-h-screen bg-gray-50 relative overflow-hidden pt-24 pb-24">
      <PageBg intensity={0.15} />
          ...
          >
            ...
            >
              ...
              <div className="flex gap-4">
                <button onClick={() => setConfirmDelete(null)} className="flex-1 py-3 rounded-2xl border-2 border-gray-200 font-black text-gray-500 hover:border-gray-300 transition-all">
                  Cancelar
                </button>
                <button
                  onClick={() => {
        ...
        {offerEdit && (
          ...
          >
            ...
            >
              ...
              />
              <div className="flex gap-4">
                <button onClick={() => setOfferEdit(null)} className="flex-1 py-3 rounded-2xl border-2 border-gray-200 font-black text-gray-500">
                  Cancelar
                </button>
                <button
        ...
        {editingProduct && (
          ...
          >
            ...
            >
              <button 
                onClick={() => { setEditingProduct(null); setImagePreview(null); }}
                className="absolute top-6 right-6 p-2 bg-gray-100 rounded-xl text-gray-400 hover:text-red-500 transition-colors"
               >
                <X size={20} />
              </button>
              ...
              <div className="space-y-3">
                  ...
                  <div className="relative h-32 w-full border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center overflow-hidden transition-all bg-gray-50 hover:bg-blue-50 mt-2">
                    {imagePreview ? (
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="overflow-x-auto pb-2 mb-8">
          <div className="flex bg-white p-1.5 rounded-3xl shadow-sm border border-gray-200 gap-1 min-w-max">
            {TABS.map(tab => (
              <button
                key={tab.id}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-pink-500 to-rose-900 rounded-[2.5rem] p-6 md:p-8 shadow-xl text-white relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col lg:flex-row gap-8 items-center justify-between">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <input type="number" placeholder="Ej. 100"
                        className="w-full p-4 bg-slate-900/10 border border-white/20 rounded-2xl outline-none text-white font-black text-lg focus:bg-slate-900/20 transition-all placeholder:text-white/30"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <input type="number" placeholder="Ej. 30"
                          className="w-full p-4 bg-slate-900/10 border border-white/20 rounded-2xl outline-none text-white font-black text-lg focus:bg-slate-900/20 transition-all placeholder:text-white/30 pl-12"
                    ...
                    <div className="space-y-2 flex flex-col justify-center">
                       ...
                       <button onClick={() => setCalcIgv(!calcIgv)}
                         className={`py-4 rounded-2xl font-black text-sm transition-all border-2 ${calcIgv ? 'bg-slate-900 text-cyan-400 border-white' : 'bg-transparent text-blue-200 border-blue-400 hover:bg-slate-900/10'}`}>
                ...
                <div className="bg-slate-900/10 backdrop-blur-md rounded-3xl p-6 border border-white/20 w-full lg:w-72 shrink-0">
                  ...
                  <button 
                    ...
                    className="w-full py-4 bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl font-black shadow-lg hover:shadow-blue-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                    {calcTargetId ? "Aplicar Precio Sugerido" : "Seleccionar un producto"}
                  </button>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex bg-gray-100 p-1 rounded-xl border border-gray-200">
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg text-xs font-black transition-all flex items-center gap-2 ${inventoryView === "table" ? "bg-blue-infositel text-white shadow-md" : "text-gray-400 hover:text-gray-700"}`}
                    >
                      <ClipboardList size={14} /> Tabla
                    </button>
                    <button 
                      onClick={() => setInventoryView("showcase")}
                      className={`px-4 py-2 rounded-lg text-xs font-black transition-all flex items-center gap-2 ${inventoryView === "showcase" ? "bg-blue-infositel text-white shadow-md" : "text-gray-400 hover:text-gray-700"}`}
                    >
              ...
              {inventoryView === "table" ? (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[700px]">
                    <thead>
                      <tr className="border-b border-gray-200 text-xs font-black text-gray-400 uppercase tracking-widest">
                          ...
                          <tr key={p.id} className="border-b border-gray-100 hover:bg-blue-50/30">
                            ...
                            <td className="py-3 px-4 text-center">
                              <button 
                                ...
                                className={`p-2.5 rounded-xl transition-all font-black text-xs mr-2 ${calcTargetId === p.id ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-500 hover:bg-blue-100'}`}
                      ...
                      >
                         ...
                         <div className="absolute top-6 right-6 z-20 flex flex-col gap-2">
                            <button 
                               ...
                               className="p-3 bg-white/90 backdrop-blur-md text-gray-400 rounded-2xl hover:text-blue-infositel hover:scale-110 transition-all border border-gray-200 shadow-sm"
                             >
                               <Edit3 size={18} />
                             </button>
                             <button 
                               ...
                               className={`p-3 backdrop-blur-md rounded-2xl hover:scale-110 transition-all border shadow-sm ${calcTargetId === p.id ? 'bg-blue-infositel text-white border-blue-infositel' : 'bg-white/90 text-blue-infositel border-gray-200'}`}
                             >
                         ...
                         <div className="space-y-6">
                           <div>
                             ...
                             <div className="mt-3 relative">
                               ...
                               {p.description.length > 50 && (
                                  <button onClick={() => toggleInventario(p.id)} className="text-[10px] font-black text-cyan-400/40 mt-1 hover:text-cyan-400 transition-colors flex items-center gap-1 group/expand">
                           ...
                           <div className="flex items-center gap-3 pt-2">
                               <div className="bg-gray-50 p-3 rounded-2xl border border-gray-200 flex-1">
                                 <p className="text-[9px] font-black text-gray-400 uppercase mb-0.5 tracking-widest">Costo Unit.</p>
                                 <p className="font-bold text-gray-700">S/. {(p.costPrice || 0).toFixed(2)}</p>
                               </div>
                               <button 
                                 onClick={() => quickSale(p)}
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 space-y-3">
                    <div className="flex items-center justify-between">
                      <label className="text-sm font-black text-blue-infositel">¿Activar Oferta?</label>
                      <button
                  ...
                  >
                    {isUploading ? (
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-[10px] font-black text-blue-infositel animate-pulse text-center">PROCESANDO<br/>IMAGEN...</p>
                      </div>
                    ) : imagePreview ? (
                      <div className="relative h-full w-full">
                        <Image src={imagePreview} alt="Preview" fill className="object-contain p-4" />
                        <div className="absolute inset-x-0 bottom-0 bg-blue-600/90 backdrop-blur-sm p-2 text-center">
                           <p className="text-white text-[9px] font-black uppercase">Click para cambiar imagen ✅</p>
                        </div>
                      </div>
                    ) : (
                      <div className="text-center space-y-3">
                        <div className="w-10 h-10 bg-white rounded-xl shadow-sm border border-gray-100 flex items-center justify-center mx-auto">
              ...
              <div className="space-y-3">
                {products.map(p => (
                  <div key={p.id} className="flex items-center gap-3 md:gap-4 p-3 md:p-4 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-colors">
                    <div className="relative h-14 w-14 bg-gray-50 rounded-xl shrink-0 overflow-hidden border border-gray-200">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="font-black text-sm truncate text-gray-900">{p.name}</h4>
                    ...
                    <button
                      ...
                      className="p-2.5 bg-blue-50 text-blue-400 rounded-xl hover:bg-blue-600 hover:text-white transition-all shrink-0"
                    >
                      <Edit3 size={16} />
                    </button>
                    {/* Delete button */}
            ...
            <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-gray-200">
             ...
             <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-gray-200">
              <div className="flex flex-wrap gap-3">
                    ...
                    {editingCategory === c.id ? (
                      <div className="flex items-center gap-2">
                        <input 
                          autoFocus
                          type="text" 
                          className="bg-white border border-blue-200 rounded-lg px-2 py-1 text-sm font-bold outline-none uppercase text-gray-900"
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => { setEditingCategory(c.id); setEditCategoryValue(c.name); }}
                            className="text-gray-400 hover:text-blue-infositel transition-colors">
                            <Edit3 size={14} />
                          </button>
                          {c.name !== "Todos" && (
                ...
                {categories.length === 0 && (
                  <div className="text-center py-10 text-gray-400 font-bold w-full">No hay categorías.</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-sm border border-gray-200">
              ...
              <div className="space-y-4">
                  ...
                  <div key={r.id} className="p-4 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div>
                        ...
                        <h4 className="font-black text-sm text-gray-900">{r.equipment}</h4>
                ...
                {repairs.length === 0 && (
                  <div className="text-center py-16 text-gray-400 font-bold">No hay equipos registrados.</div>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="space-y-3">
                {adminUsers.map(u => (
                  <div key={u.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl ${u.role === "superadmin" ? "bg-blue-50 text-blue-infositel" : "bg-gray-100 text-gray-400"}`}>
                        <User size={18} />
                      </div>
                      <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               {[
                 ...
                 >
                   ...
                   <h3 className="text-3xl font-black mb-2 text-gray-900">S/. {s.val?.toFixed(2)}</h3>
                   <div className="flex items-center gap-2">
                 ...
                 <div className="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-sm border border-gray-200">
                    ...
                    <div className="space-y-4">
                         ...
                         <div key={s.id} className="p-5 border border-gray-100 rounded-[1.8rem] hover:bg-gray-50 transition-all flex items-center justify-between group">
                            <div className="flex items-center gap-4">
                               ...
                               <div>
                                  ...
                                  <h4 className="font-black text-sm text-gray-900">{s.pName} <span className="text-blue-infositel/50 ml-1">x{s.quantity}</span></h4>
                                  <div className="flex items-center gap-2 mt-1">
                                     <span className="text-[10px] font-black text-blue-infositel bg-blue-50 px-2 py-0.5 rounded-lg">GANANCIA: S/. {s.profit.toFixed(2)}</span>
                                  </div>
                               </div>
                            </div>
                            <div className="flex items-center gap-4">
                               <div className="text-right">
                                  <span className="text-lg font-black block text-gray-900">S/. {(s.price * s.quantity).toFixed(2)}</span>
                                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Tienda</span>
                               </div>
                               <button onClick={() => setConfirmDelete({ type: "sale", id: s.id })} className="p-2 text-gray-300 hover:text-red-500 transition-all">
                                  <Trash2 size={16} />
                               </button>
                            </div>
                         </div>
                         ...
                         <div key={o.id} className="p-5 border border-green-100 bg-green-50/30 rounded-[1.8rem] hover:bg-green-50/50 transition-all flex items-center justify-between group">
                            <div className="flex items-center gap-4">
                               ...
                               <div>
                                  ...
                                  <h4 className="font-black text-sm text-gray-900">{o.customerName}</h4>
                                  <p className="text-[10px] font-black text-green-500/60 uppercase">{o.customerPhone}</p>
                               </div>
                            </div>
                            <div className="flex items-center gap-4">
                               <div className="text-right">
                                  <span className="text-lg font-black block text-gray-900">S/. {o.total.toFixed(2)}</span>
                                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Web / WhatsApp</span>
                               </div>
                       ...
                       {sales.length === 0 && orders.length === 0 && (
                         <div className="text-center py-20 text-gray-400 font-black italic">Sin movimientos registrados hoy.</div>
                       )}
                    </div>
                 </div>
              </div>
