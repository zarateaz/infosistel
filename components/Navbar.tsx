...
import { ShoppingCart, User, Facebook, MessageCircle, Menu, X } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 bg-white/85 backdrop-blur-xl ${
        isScrolled ? "py-2 shadow-[0_8px_40px_rgba(20,51,201,0.06)]" : "py-4"
      }`}
    >
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center group">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <Logo width={190} className="hidden md:flex" />
          </motion.div>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => (
            ...
            >
              {link.name}
              <motion.span
                className="absolute bottom-0 left-1/2 h-[2px] rounded-full bg-gradient-brand"
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 mr-2">
            {socialLinks.map((social, idx) => (
              <a
                ...
                className={`p-2 rounded-xl transition-all duration-300 hover:scale-110 ${social.color} bg-gray-50/50 hover:bg-gray-100`}
          ...
          <Link
            href="/tienda"
            className="relative p-2 rounded-xl text-gray-400 hover:text-blue-infositel hover:bg-blue-infositel/5 transition-all"
          >
        ...
        {/* Mobile Toggle */}
        <button
          className="lg:hidden p-2 rounded-xl hover:bg-gray-50 text-black transition-colors"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.25 }}
            className="lg:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-2xl shadow-[0_40px_80px_rgba(0,0,0,0.08)] p-6 flex flex-col gap-1"
          >
            {navLinks.map((link, i) => (
              <motion.div
                key={link.name}
